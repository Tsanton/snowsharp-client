...
public class DatabaseGrant : ISnowflakeGrantAsset
{
    ...
    public string GetGrantStatement(ISnowflakePrincipal principal, List<Privilege> privileges)
    {
        ...
        {
            case Role:
                return string.Format("GRANT {0} ON DATABASE {1} TO ROLE {2};", flatPrivileges, DatabaseName, principal.GetObjectIdentifier());
            case DatabaseRole:
                return string.Format("GRANT {0} ON DATABASE {1} TO DATABASE ROLE {2};", flatPrivileges, DatabaseName, principal.GetObjectIdentifier());
            default:
    ...
    public string GetRevokeStatement(ISnowflakePrincipal principal, List<Privilege> privileges)
    {
        ...
        {
            case Role:
                return string.Format("REVOKE {0} ON DATABASE {1} FROM ROLE {2} CASCADE;", flatPrivileges, DatabaseName, principal.GetObjectIdentifier());
            case DatabaseRole:
                return string.Format("REVOKE {0} ON DATABASE {1} FROM DATABASE ROLE {2} CASCADE;", flatPrivileges, DatabaseName, principal.GetObjectIdentifier());
            default:
